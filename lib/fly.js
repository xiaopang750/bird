...
let usersetConfigFileLocation = `${userHomeDir}/.fly/config.json`
...
plan.local('deploy', () => {
  let nginxConf = `${userHomeDir}/.fly/${projectName}.conf`
  let hasNignxConf = fs.existsSync(nginxConf)

  if (!hasNignxConf) {
    throw new Error('缺少nginx配置，请在~/.fly目录下配置${projectName}.conf文件 执行bird ge可自动生成')
  }
})
...
plan.remote('deploy', remote => {
  let nginxConfTemplate = fs.readFileSync(`${userHomeDir}/.fly/${projectName}.conf`, 'utf8')
